...
<?php
	...
	if ($sport == 'Baseball')
	{
		if ($playertype == 'Manager'){
			...
			$querystats2 = "SELECT distinct year, team, league, games, wins, loss, winpercent, ROUND((WINPERCENT - (SELECT AVG(WINPERCENT) FROM BASEBALLMANAGERS)) / (SELECT STDDEV(WINPERCENT) FROM BASEBALLMANAGERS), 3) zScore ,rank
						FROM Baseballmanagers
						WHERE managerid = '" . $playerid2 . "'
						ORDER BY year";
		    $querycareerstats2 = "SELECT SUM(BaseballManagers.WINS) , SUM(BaseballManagers.LOSS), ROUND((SUM(BaseballManagers.WINS) / (SUM(BaseballManagers.WINS) + SUM(BaseballManagers.LOSS))),3)
						FROM BaseballManagers
		...
		if ($playertype == 'Pitcher'){
			...
			ORDER BY year";
			$querycareerstats2 = "SELECT SUM(BaseballPitching.WINS) , SUM(BaseballPitching.LOSSES), ROUND((SUM(BaseballPitching.WINS) / (SUM(BaseballPitching.WINS) + SUM(BaseballPitching.LOSSES))),3)
			FROM BaseballPitching
			WHERE BaseballPitching.pitcherid = '" . $playerid2 . "'";
			$querypostseasonstats2 = "SELECT distinct year, round, team, league, wins, losses, ROUND(wins/(wins + losses+0.000000000000000000000000000000000000001),3) winpercent,
			games, starts, completegames, shutouts, saves, outs, hits, earnedruns, homeruns, walks, strikeouts,
			battingaverage, 
			ROUND(((battingaverage - (select avg(battingaverage) from baseballpitchingpostseason))/(select stddev(battingaverage) from baseballpitchingpostseason)),3) bavgZSCORE,
			era, ROUND(((era - (select avg(era) from baseballpitchingpostseason))/(select stddev(era) from baseballpitchingpostseason)),3) eraZSCORE,
		...
		if ($playertype == 'Team'){
			$querystats2 = "select distinct year, team, league, division, rank, games, homegames, win, loss, winpercent, 
					 ...
					 order by year";
			$querycareerstats2 = "select  SUM(games) / (COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(homegames) / (COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(win) / (COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(loss) / (COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), ROUND((SUM(win)/(SUM(win) + SUM(loss))),3) winPercent, ROUND(AVG(A.zScore), 3),
								SUM(runs)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(atbats)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(hits)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), ROUND((SUM(hits)/SUM(atbats)),3), ROUND(AVG(B.zScore),3),
								SUM(doubles)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(triples)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(homeruns)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), ROUND(((SUM(doubles) * 2 + SUM(triples) * 3 + SUM(homeruns) * 4 + (SUM(hits)-SUM(homeRuns)-SUM(triples)-SUM(doubles)))/SUM(atbats)),3), ROUND(AVG(C.zScore),3),
								SUM(walks)/ (COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(strikeouts)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(stolenbases)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(caughtstealing)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(hitbypitches)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(sacflies)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)),
								SUM(runsallowed)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)), SUM(earnedruns)/(COUNT(distinct year) * COUNT(distinct year) * COUNT(distinct year)),
								...
								from (SELECT ROUND((WINPERCENT - (SELECT AVG(WINPERCENT) FROM BASEBALLTEAMS)) / 
									 (SELECT STDDEV(WINPERCENT) FROM BASEBALLTEAMS), 3) 
									 zScore FROM BASEBALLTEAMS WHERE name = '" . $playerid2 . "')A,
									 (SELECT ROUND((battingAverage - (SELECT AVG(battingAverage) FROM BASEBALLTEAMS)) / 
									 (SELECT STDDEV(battingAverage) FROM BASEBALLTEAMS), 3) 
									 zScore FROM BASEBALLTEAMS WHERE name = '" . $playerid2 . "')B,
									 (SELECT ROUND((slugging - (SELECT AVG(slugging) FROM BASEBALLTEAMS)) / 
									 (SELECT STDDEV(slugging) FROM BASEBALLTEAMS), 3) 
									 zScore FROM BASEBALLTEAMS WHERE name = '" . $playerid2 . "')C,
									 BaseballTeams
								where name = '" . $playerid2 . "'";
		}
	}
	...
	else if ($sport == 'Basketball')
	{
		if ($playertype == 'Coach'){
			...
			$querystats2 = "SELECT distinct year, team, league, wins, losses, winningpercentage, ROUND((winningpercentage - (select avg(winningpercentage) from basketballcoaches)) / (select stddev(winningpercentage) from basketballcoaches), 3) zScore
					 FROM BASKETBALLCoaches
					 WHERE coachid = '" . $playerid2 . "'
					 order by year";
			$querycareerstats2 = "SELECT SUM(WINS), (SUM(LOSSES)), ROUND((SUM(WINS) / (SUM(LOSSES) + SUM(WINS))), 3)
						from BasketballCoaches
						WHERE BasketballCoaches.CoachID = '" . $playerid2 . "'";
			$querypostseasonstats2 = "SELECT distinct year, team, league, POSTSEASONWINS, POSTSEASONLOSSES, POSTSEASONWINNINGPERCENTAGE, ROUND((POSTSEASONWINNINGPERCENTAGE - (SELECT AVG(POSTSEASONWINNINGPERCENTAGE) FROM BASKETBALLCOACHES)) / (SELECT STDDEV(POSTSEASONWINNINGPERCENTAGE) FROM BASKETBALLCOACHES), 3) zScore
						    FROM BASKETBALLCOACHES
						    WHERE BasketballCoaches.CoachID = '" . $playerid2 . "' AND POSTSEASONWINNINGPERCENTAGE != -1
						    order by year";
			$querycareerpostseasonstats2 = "SELECT SUM(POSTSEASONWINS), SUM(POSTSEASONLOSSES), ROUND((SUM(POSTSEASONWINS) / (SUM(POSTSEASONLOSSES) + SUM(POSTSEASONWINS))), 3)
							   FROM BasketballCoaches
						          WHERE BasketballCoaches.CoachID = '" . $playerid2 . "'";
		}
		if ($playertype == 'Player'){
			$querybio2 = "SELECT firstName, lastName, height, weight
				      FROM BASKETBALLMASTER
				      WHERE id = '" . $playerid2 . "'";
			$querystats2 = "SELECT distinct year, team, league, games, minutes, points, pointspergame, 
							ROUND(((pointspergame - (select avg(pointspergame) from basketballplayers))/(select stddev(pointspergame) from basketballplayers)),3) bavgZSCORE,
							offensiverebounds, defensiverebounds, rebounds, reboundspergame,
							ROUND(((reboundspergame - (select avg(reboundspergame) from basketballplayers))/(select stddev(reboundspergame) from basketballplayers)),3),
							assists, assistspergame, 
			...
			$queryallstarstats2 = "select distinct year, conference, league, minutes, points, offensiverebounds, defensiverebounds, rebounds, assists, steals,
									...
									threeattempts, threemade
									from basketballplayerallstar
									where basketballplayerallstar.player_id = '" . $playerid2 . "'
									order by year";
			$querycareerallstarstats2 = "select sum(minutes), sum(points), sum(offensiverebounds), sum(defensiverebounds), sum(rebounds), sum(assists), sum(steals),
										sum(blocks), sum(turnovers), sum(fouls), sum(fieldgoalattempts), sum(fieldgoalmade),
										sum(freethrowattempts), sum(freethrowmade), 
										sum(threeattempts), sum(threemade)
										from basketballplayerallstar
		...
		if ($playertype == 'Team'){
			$querystats2 = "select distinct year, team, league, conference, division, rank, conferencerank, fieldgoalmade, fieldgoalattempts, fieldgoalpercent,
							ROUND(((fieldgoalpercent - (SELECT AVG(fieldgoalpercent) FROM BasketballTeams))/(SELECT STDDEV(fieldgoalpercent) FROM BasketballTeams)), 3) zScore,
							freethrowmade, freethrowattempt, freethrowpercent, 
							ROUND(((freethrowpercent - (SELECT AVG(freethrowpercent) FROM BasketballTeams))/(SELECT STDDEV(freethrowpercent) FROM BasketballTeams)), 3) zScore2,
							threemade, threeattempts, threepercent,
							...
							where name = '" . $playerid2 . "'
							order by year";
			$querycareerstats2 = "select sum(games), sum(fieldgoalmade), sum(fieldgoalattempts), (ROUND(sum(fieldgoalmade) / sum(fieldgoalattempts), 3)) fieldGoalPercent,
								sum(freethrowmade), sum(freethrowattempt), (ROUND(sum(freethrowmade) / sum(freethrowattempt), 3)) freethrowPercent,
								sum(threemade), sum(threeattempts), (ROUND(sum(threemade) / sum(threeattempts), 3)) threePercent,
								sum(offensiverebounds), sum(defensiverebounds), sum(rebounds), (ROUND(sum(rebounds) / sum(games), 3)) reboundspergame,
								sum(assists), (ROUND(sum(assists) / sum(games), 3)) assistspergame, sum(fouls), sum(steals), sum(turnovers), sum(blocks), sum(points), (ROUND(sum(points) / sum(games), 3)) pointspergame,
	...
	else if ($sport == 'Hockey')
	{
		if ($playertype == 'Coach'){
			...
			$querystats2 = "SELECT DISTINCT YEAR, TEAM, LEAGUE, GAMES, WIN, LOSS, TIE, POINTSPERGAME, ROUND(((POINTSPERGAME - (SELECT AVG(POINTSPERGAME) FROM HOCKEYCOACHES))/(SELECT STDDEV(POINTSPERGAME) FROM HOCKEYCOACHES)), 3) zScore
			   FROM HOCKEYCOACHES
			   WHERE COACHID = '" . $playerid2 . "'
			   ORDER BY YEAR";
			$querycareerstats2 = "SELECT SUM(WIN) / (4 * COUNT(DISTINCT YEAR)), (SUM(LOSS) / (4 * COUNT(DISTINCT YEAR))), (SUM(TIE) / (4 * COUNT(DISTINCT YEAR))), ROUND(((2 * SUM(WIN) + SUM(TIE)) / (SUM(LOSS) + SUM(WIN) + SUM(TIE))), 3), ROUND(AVG(A.zScore), 3)
					 FROM (SELECT ROUND (((POINTSPERGAME - (SELECT AVG(POINTSPERGAME) FROM HOCKEYCOACHES)) / 
					(SELECT STDDEV(POINTSPERGAME) FROM HOCKEYCOACHES)), 3) zScore FROM HockeyCoaches WHERE COACHID = '" . $playerid2 . "')A, HockeyCoaches
					 WHERE COACHID = '" . $playerid2 . "'";
			$querypostseasonstats2 = "SELECT distinct year, team, league, postseasongames, postseasonwin, postseasonloss, postseasontie, postseasonwinpercent						    
						 from hockeycoaches
						 WHERE COACHID = '" . $playerid2 . "' AND postseasonwinpercent != -1
						 order by year";
			$querycareerpostseasonstats2 = "SELECT SUM(POSTSEASONWIN), SUM(POSTSEASONLOSS), SUM(POSTSEASONTIE), ROUND((SUM(POSTSEASONWIN) / (SUM(POSTSEASONLOSS) + SUM(POSTSEASONWIN))), 3)
							   FROM HockeyCoaches
							   WHERE COACHID = '" . $playerid2 . "'";
		}
		if ($playertype == 'Goalie'){
			...
			$querystats2 = "select distinct year, team, league, games, minutes, wins, loss, ties, pointspergame, ROUND(((pointspergame - (select avg(pointspergame) from hockeygoalies))/(select stddev(pointspergame) from hockeygoalies)),3) bavgZSCORE,
							...
							order by year";
			$querycareerstats2 = "select sum(games), sum(minutes), sum(wins), sum(loss), sum(ties), 
								sum(emptynetgoals), sum(shutouts), sum(goalsallowed), sum(shotsallowed),ROUND(((1 - (SUM(GOALSALLOWED)/SUM(SHOTSALLOWED)))),3)
								from hockeygoalies
								where hockeygoalies.goalieid = '" . $playerid2 . "'";
			$querypostseasonstats2 = "select distinct year, team, league, postseasongames, postseasonminutes, postseasonwins, postseasonloss, postseasonwinpercent,
									ROUND(((postseasonwinpercent - (select avg(postseasonwinpercent) from hockeygoalies))/(select stddev(postseasonwinpercent) from hockeygoalies)),3),
									postseasonemptynetgoals, postseasonshutouts, postseasongoalsallowed, postseasonshotsallowed, postseasonsavepercent,
									ROUND(((postseasonsavepercent - (select avg(postseasonsavepercent) from hockeygoalies))/(select stddev(postseasonsavepercent) from hockeygoalies)),3)
									from hockeygoalies
									...
									order by year";
			$querycareerpostseasonstats12 = "select sum(postseasongames), sum(postseasonminutes), sum(postseasonwins), sum(postseasonloss),
											ROUND((sum(postseasonwins)/(sum(postseasonwins) + sum(postseasonloss))),3),
											sum(postseasonemptynetgoals), sum(postseasonshutouts), sum(postseasongoalsallowed), sum(postseasonshotsallowed), 
											ROUND((1 - (sum(postseasongoalsallowed)/(sum(postseasonshotsallowed)))),3)
											from hockeygoalies
											where hockeygoalies.goalieid = '" . $playerid2 . "'";
			$queryshootoutstats2 = "select distinct year, team, wins, loss, winpercent, 
									ROUND(((winpercent - (select avg(winpercent) from hockeygoaliesshootout))/(select stddev(winpercent) from hockeygoaliesshootout)),3),
									goalsallowed, shotsallowed, savepercent,
									ROUND(((savepercent - (select avg(savepercent) from hockeygoaliesshootout))/(select stddev(savepercent) from hockeygoaliesshootout)),3)
									from hockeygoaliesshootout
									...
									order by year";
			$querycareershootoutstats2 = "select sum(wins), sum(loss), ROUND((sum(wins)/(sum(wins) + sum(loss))),3),
										sum(goalsallowed), sum(shotsallowed), ROUND((1 - (sum(goalsallowed)/(sum(shotsallowed)))),3)
										from hockeygoaliesshootout
										where hockeygoaliesshootout.goalieid = '" . $playerid2 . "'";
		...
		if ($playertype == 'Team'){
			$querystats2 = "select distinct year, team, league, conference, division, rank, games, win, loss, points,  goals, ROUND(((goals - (select avg(goals) from hockeyteams))/(select stddev(goals) from hockeyteams)),3) zScore,
						goalsallowed, ROUND(((goalsallowed - (select avg(goalsallowed) from hockeyteams))/(select stddev(goalsallowed) from hockeyteams)),3) zScore2, goaldifferential,
						penaltyminutes, benchminutes, powerplaygoals, powerplaychances, shorthandedgoalsallowed, penaltykillgoals, penaltykillchances, shorthandedgoals
						from hockeyteams
						where name = '" . $playerid2 . "'
						order by year";
			$querycareerstats2 = "select sum(games)/(count(distinct year) * count(distinct year)), sum(win)/(count(distinct year) * count(distinct year)), sum(loss)/(count(distinct year) * count(distinct year)),
								sum(points)/(count(distinct year) * count(distinct year)), sum(goals)/(count(distinct year) * count(distinct year)), ROUND((AVG(A.zScore)),3), sum(goalsallowed)/(count(distinct year) * count(distinct year)),
								ROUND((AVG(B.zScore)),3), (sum(goals) - sum(goalsallowed))/(count(distinct year) * count(distinct year)), sum(penaltyminutes)/(count(distinct year) * count(distinct year)),
								sum(benchminutes)/(count(distinct year) * count(distinct year)), sum(powerplaygoals)/(count(distinct year) * count(distinct year)), sum(powerplaychances)/(count(distinct year) * count(distinct year)),
								sum(shorthandedgoalsallowed)/(count(distinct year) * count(distinct year)),sum(penaltykillgoals)/(count(distinct year) * count(distinct year)), sum(penaltykillchances)/(count(distinct year) * count(distinct year)),
								...
								from(SELECT ROUND((goals - (SELECT AVG(goals) FROM HOCKEYTEAMS)) / 
								     						(SELECT STDDEV(goals) FROM HOCKEYTEAMS), 3) 
								      						zScore FROM HOCKEYTEAMS WHERE name = '" . $playerid2 . "')A,
								     (SELECT ROUND((goalsallowed - (SELECT AVG(goalsallowed) FROM HOCKEYTEAMS)) / 
								     						(SELECT STDDEV(goalsallowed) FROM HOCKEYTEAMS), 3) 
								      						zScore FROM HOCKEYTEAMS WHERE name = '" . $playerid2 . "')B, hockeyteams
								where name = '" . $playerid2 . "'";
			$querypostseasonstats2 = "select distinct HockeyTeamsPostseason.*, ROUND(((HockeyTeamsPostseason.winpercent - (select avg(winpercent) from hockeyteamspostseason))/(select stddev(winpercent) from hockeyteamspostseason)),3) zScore,
									ROUND(((HockeyTeamsPostseason.goals - (select avg(goals) from hockeyteamspostseason))/(select stddev(goals) from hockeyteamspostseason)),3) zScore2,
									ROUND(((HockeyTeamsPostseason.goalsallowed - (select avg(goalsallowed) from hockeyteamspostseason))/(select stddev(goalsallowed) from hockeyteamspostseason)),3) zScore3
									from HockeyTeamsPostseason, hockeyteams
									where HockeyTeamsPostseason.team = hockeyteams.team AND hockeyteams.name = '" . $playerid2 . "'
									order by HockeyTeamsPostseason.year";
			$querysplitstats2 = "select distinct HockeyTeamsSplit.year, HockeyTeamsSplit.league, HockeyTeamsSplit.team, HockeyTeamsSplit.homewin, HockeyTeamsSplit.homeloss, HockeyTeamsSplit.homepointspergame,
								HockeyTeamsSplit.roadwin, HockeyTeamsSplit.roadloss, HockeyTeamsSplit.roadpointspergame, HockeyTeamsSplit.januarywin, HockeyTeamsSplit.januaryloss, HockeyTeamsSplit.januarypointspergame,
								HockeyTeamsSplit.februarywin, HockeyTeamsSplit.februaryloss, HockeyTeamsSplit.februarypointspergame, HockeyTeamsSplit.marchwin, HockeyTeamsSplit.marchloss, HockeyTeamsSplit.marchpointspergame,
								HockeyTeamsSplit.aprilwin, HockeyTeamsSplit.aprilloss, HockeyTeamsSplit.aprilpointspergame
								from HockeyTeamsSplit, HockeyTeams
								where HockeyTeamsSplit.team = HockeyTeams.team AND HockeyTeams.name = '" . $playerid2 . "'
								order by HockeyTeamsSplit.year";
			$querycareersplitstats2 = "select sum(HockeyTeamsSplit.homewin)/(count(distinct HockeyTeamsSplit.year)), sum(HockeyTeamsSplit.homeloss)/ count(distinct HockeyTeamsSplit.year) , ROUND((sum(HockeyTeamsSplit.homewin)/(sum(HockeyTeamsSplit.homewin) + sum(HockeyTeamsSplit.homeloss))),3),
										sum(HockeyTeamsSplit.roadwin)/(count(distinct HockeyTeamsSplit.year)), sum(HockeyTeamsSplit.roadloss)/ count(distinct HockeyTeamsSplit.year) , ROUND((sum(HockeyTeamsSplit.roadwin)/(sum(HockeyTeamsSplit.roadwin) + sum(HockeyTeamsSplit.roadloss))),3),
										sum(HockeyTeamsSplit.januarywin)/(count(distinct HockeyTeamsSplit.year)), sum(HockeyTeamsSplit.januaryloss)/ count(distinct HockeyTeamsSplit.year) , ROUND((sum(HockeyTeamsSplit.januarywin)/(sum(HockeyTeamsSplit.januarywin) + sum(HockeyTeamsSplit.januaryloss))),3),
										sum(HockeyTeamsSplit.februarywin)/(count(distinct HockeyTeamsSplit.year)), sum(HockeyTeamsSplit.februaryloss)/ count(distinct HockeyTeamsSplit.year) , ROUND((sum(HockeyTeamsSplit.februarywin)/(sum(HockeyTeamsSplit.februarywin) + sum(HockeyTeamsSplit.februaryloss))),3),
										sum(HockeyTeamsSplit.marchwin)/(count(distinct HockeyTeamsSplit.year)), sum(HockeyTeamsSplit.marchloss)/ count(distinct HockeyTeamsSplit.year) , ROUND((sum(HockeyTeamsSplit.marchwin)/(sum(HockeyTeamsSplit.marchwin) + sum(HockeyTeamsSplit.marchloss))),3),
	...
	{
		$statementcareerstats2 = oci_parse($connection, $querycareerstats2);
		oci_execute($statementcareerstats2);
		echo "<table border='1'>\n";
		if($sport == 'Baseball')
		{
			if($playertype == 'Manager')
		...
		while ($row = oci_fetch_array($statementcareerstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
			echo "<tr>\n";
			foreach ($row as $item) {
				echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
			}
	...
	$statementcareerpostseasonstats2 = oci_parse($connection, $querycareerpostseasonstats2);
	if($playertype == 'Team' || ($sport == 'Hockey' && $playertype == 'Goalie') || ($sport == 'Hockey' && $playertype == 'Position Player'))
	{
	...
	else
	{
		if(($sport == 'Baseball' && $playertype == 'Pitcher') || ($sport == 'Basketball' && $playertype == 'Coach') || ($sport == 'Hockey' && $playertype = 'Coach') || ($sport == 'Basketball' && $playertype == 'Player'))
		{
			oci_execute($statementcareerpostseasonstats2);
			echo "<table border='1'>\n";
			if($sport == 'Baseball')
			{
				if($playertype == 'Manager'){
			...
			while ($row = oci_fetch_array($statementcareerpostseasonstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
				echo "<tr>\n";
				foreach ($row as $item) {
					echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
				}
	...
	$statementsplitstats2 = oci_parse($connection, $querysplitstats2);
	if($sport == 'Hockey' && $playertype == 'Team')
	{
		echo '<font size = "4"><b>Team Split Stats</b></font><br>';
		oci_execute($statementsplitstats2);
		echo "<table border='1'>\n";
		echo '<tr>';
				echo '<th>Year</th>';
				echo '<th>League</th>';
				...
				echo '<th>April Points Per Game</th>';
				echo '</tr>';
		while ($row = oci_fetch_array($statementsplitstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
			echo "<tr>\n";
				foreach ($row as $item) {
					echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
				}
	...
	$statementcareersplitstats2 = oci_parse($connection, $querycareersplitstats2);
	if($sport == 'Hockey' && $playertype == 'Team')
	{
		echo '<font size = "4"><b>Team Career Split Stats</b></font><br>';
		oci_execute($statementcareersplitstats2);
		echo "<table border='1'>\n";
		echo '<tr>';
				echo '<th>Total Home Wins</th>';
				echo '<th>Total Home Losses</th>';
				...
				echo '<th>Total April Win Percent</th>';
				echo '</tr>';
		while ($row = oci_fetch_array($statementcareersplitstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
			echo "<tr>\n";
				foreach ($row as $item) {
					echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
				}
	...
	$statementcareerbattingstats2 = oci_parse($connection, $querycareerbattingstats2);
	if($sport == 'Baseball' && ($playertype == 'Pitcher' || $playertype == 'Position Player'))
	{
		echo '<font size = "4"><b>Regular Season Career Batting Stats</b></font><br>';
		oci_execute($statementcareerbattingstats2);
		echo "<table border='1'>\n";
			    echo '<tr>';
				echo '<th>Total Games</th>';
				echo '<th>Total Starts</th>';
				...
				echo '<th>Total GIDP</th>';
				echo '</tr>';
		while ($row = oci_fetch_array($statementcareerbattingstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
			echo "<tr>\n";
				foreach ($row as $item) {
					echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
				}
	...
	$statementcareerpostseasonbattingstats2 = oci_parse($connection, $querycareerpostseasonbattingstats2);
	if($sport == 'Baseball' && ($playertype == 'Pitcher' || $playertype == 'Position Player'))
	{
		echo '<font size = "4"><b>Career Postseason Batting Stats</b></font><br>';
		oci_execute($statementcareerpostseasonbattingstats2);
		echo "<table border='1'>\n";
			    echo '<tr>';
				echo '<th>Total Games</th>';
				echo '<th>Total At-bats</th>';
				...
				echo '<th>Total GIDP</th>';
				echo '</tr>';
		while ($row = oci_fetch_array($statementcareerpostseasonbattingstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
			echo "<tr>\n";
				foreach ($row as $item) {
					echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
				}
	...
	$statementcareerfieldingstats2 = oci_parse($connection, $querycareerfieldingstats2);
	if($sport == 'Baseball' && ($playertype == 'Pitcher' || $playertype == 'Position Player'))
	{
		echo '<font size = "4"><b>Career Regular Season Fielding Stats</b></font><br>';
		oci_execute($statementcareerfieldingstats2);
		echo "<table border='1'>\n";
		echo "<tr>\n";
				echo '<th>Total Games</th>';
				echo '<th>Toal Starts</th>';
				...
				echo '<th>Total Fielding Percent</th>';
				echo '</tr>';	   
		while ($row = oci_fetch_array($statementcareerfieldingstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementcareerpostseasonfieldingstats2 = oci_parse($connection, $querycareerpostseasonfieldingstats2);
	if($sport == 'Baseball' && ($playertype == 'Pitcher' || $playertype == 'Position Player'))
	{
		echo '<font size = "4"><b>Career Postseason Fielding Stats</b></font><br>';
		oci_execute($statementcareerpostseasonfieldingstats2);
		echo "<table border='1'>\n";
		echo "<tr>\n";
				echo '<th>Total Games</th>';
				echo '<th>Toal Starts</th>';
				...
				echo '<th>Total Fielding Percent</th>';
				echo '</tr>';	   
		while ($row = oci_fetch_array($statementcareerpostseasonfieldingstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementallstarstats2 = oci_parse($connection, $queryallstarstats2);
	if($sport == 'Basketball' && $playertype == 'Player')
	{
		echo '<font size = "4"><b>All-Star Game Stats</b></font><br>';
		oci_execute($statementallstarstats2);
		echo "<table border='1'>\n";
		echo "<tr>\n";
		echo '<th>Year</th>';
		echo '<th>Conference</th>';
		...
		echo '<th>Three Pointers Made</th>';
		echo '</tr>';	   
		while ($row = oci_fetch_array($statementallstarstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementcareerallstarstats2 = oci_parse($connection, $querycareerallstarstats2);
	if($sport == 'Basketball' && $playertype == 'Player')
	{
		echo '<font size = "4"><b>Career All-Star Game Stats</b></font><br>';
		oci_execute($statementcareerallstarstats2);
		echo "<table border='1'>\n";
		echo "<tr>\n";
		echo '<th>Total Minutes</th>';
		echo '<th>Total Points</th>';
		...
		echo '<th>Total Three Pointers Made</th>';
		echo '</tr>';	   
		while ($row = oci_fetch_array($statementcareerallstarstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementcareerpostseasonstats21 = oci_parse($connection, $querycareerpostseasonstats21);
	if($sport == 'Hockey' && $playertype == 'Goalie')
	{
		echo '<font size = "4"><b>Career Postseason Goaltending Stats</b></font><br>';
		oci_execute($statementcareerpostseasonstats21);
		echo "<table border='1'>\n";
		echo '<tr>';
					echo '<th>Total Games</th>';
					echo '<th>Total Minutes</th>';
					...
					echo '<th>Total Save Percent</th>';
					echo '</tr>';   
		while ($row = oci_fetch_array($statementcareerpostseasonstats21, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementcareershootoutstats2 = oci_parse($connection, $querycareershootoutstats2);
	if($sport == 'Hockey' && $playertype == 'Goalie')
	{
		oci_execute($statementcareershootoutstats2);
				echo '<font size = "4"><b>Career Shootout Goaltending Stats</b></font><br>';
				echo "<table border='1'>\n";
				echo '<tr>';
				echo '<th>Total Wins</th>';
				...
				echo '<th>Total Save Percent</th>';
				echo '</tr>'; 
		while ($row = oci_fetch_array($statementcareershootoutstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementcareerscoringstats2 = oci_parse($connection, $querycareerscoringstats2);
	if(($sport == 'Hockey' && $playertype == 'Goalie') || ($sport == 'Hockey' && $playertype == 'Position Player'))
	{
		oci_execute($statementcareerscoringstats2);
		echo '<font size = "4"><b>Career Regular Season Scoring Stats</b></font><br>';
		echo "<table border='1'>\n";
		echo '<tr>';
		echo '<th>Total Games</th>';
		...
		echo '<th>Total Shot Percent</th>';
		echo '</tr>';
		while ($row = oci_fetch_array($statementcareerscoringstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementpostseasonscoringstats2 = oci_parse($connection, $querypostseasonscoringstats2);
	if(($sport == 'Hockey' && $playertype == 'Goalie') || ($sport == 'Hockey' && $playertype == 'Position Player'))
	{
		oci_execute($statementpostseasonscoringstats2);
		echo '<font size = "4"><b>Postseason Scoring Stats</b></font><br>';
		echo "<table border='1'>\n";
		echo '<tr>';
		echo '<th>Year</th>';
		...
		echo '<th>Shot Percent</th>';
		echo '</tr>';
		while ($row = oci_fetch_array($statementpostseasonscoringstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementcareerpostseasonscoringstats2 = oci_parse($connection, $querycareerpostseasonscoringstats2);
	if(($sport == 'Hockey' && $playertype == 'Goalie') || ($sport == 'Hockey' && $playertype == 'Position Player'))
	{
		oci_execute($statementcareerpostseasonscoringstats2);
		echo '<font size = "4"><b>Career Postseason Scoring Stats</b></font><br>';
		echo "<table border='1'>\n";
		echo '<tr>';
		echo '<th>Total Games</th>';
		...
		echo '<th>Total Shot Percent</th>';
		echo '</tr>';
		while ($row = oci_fetch_array($statementcareerpostseasonscoringstats2, OCI_ASSOC+OCI_RETURN_NULLS)) {
	...
	$statementshootoutstats21 = oci_parse($connection, $queryshootoutstats21);
	if($sport == 'Hockey' && $playertype == 'Position Player')
	{
				oci_execute($statementshootoutstats21);
				echo '<font size = "4"><b>Shootout Scoring Stats</b></font><br>';
				echo "<table border='1'>\n";
				echo '<tr>';
				echo '<th>Year</th>';
				...
				echo '<th>Shot Percent z-Score</th>';
				echo '</tr>'; 
		while ($row = oci_fetch_array($statementshootoutstats21, OCI_ASSOC+OCI_RETURN_NULLS)) {
				foreach ($row as $item) {
					echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
				}
				echo "</tr>\n";
	...
	$statementcareershootoutstats21 = oci_parse($connection, $querycareershootoutstats21);
	if($sport == 'Hockey' && $playertype == 'Position Player')
	{
				oci_execute($statementcareershootoutstats21);
				echo '<font size = "4"><b>Career Shootout Scoring Stats</b></font><br>';
				echo "<table border='1'>\n";
				echo '<tr>';
				echo '<th>Total Shots</th>';
				...
				echo '<th>Total Shot Percent</th>';
				echo '</tr>'; 
		while ($row = oci_fetch_array($statementcareershootoutstats21, OCI_ASSOC+OCI_RETURN_NULLS)) {
				foreach ($row as $item) {
					echo "    <td>" . ($item !== null ? htmlentities($item, ENT_QUOTES) : "&nbsp;") . "</td>\n";
				}
				echo "</tr>\n";
